...
export type ShowcaseTour = {
  ...
  address: string | null;
  city: string | null;
  /** Koordinate iz geokodiranja adrese (migracija 016) - null dok se ne izračunaju. */
  lat: number | null;
  lng: number | null;
...
type TourRow = {
  ...
  city?: string | null;
  /** Postoje tek posle migracije 016; zatečene ture su null dok se ne sačuvaju ponovo. */
  lat?: number | null;
  lng?: number | null;
...
export async function getShowcaseTours(lang = 'sr'): Promise<ShowcaseTour[]> {
  ...
  return tours.map((tour) => {
    ...
    return {
      ...
      lat: asNumber(tour.lat),
      lng: asNumber(tour.lng),
